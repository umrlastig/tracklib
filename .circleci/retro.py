...
# Removes static variable types [ADDED 20/01]
		...
		for f2 in F2:
			...
			for line in L:
				...
				if (": float = " in line) and ("#:" in line):
					line = line.replace(": float =", " = ")[:-1] + " [|float|]\n"
					print("  Removing: static type float")
				if (": int = " in line) and ("#:" in line):
					line = line.replace(": int =", " = ")[:-1] + " [|int|]\n"
					print("  Removing: static type int")
				if (": str = " in line) and ("#:" in line):
					line = line.replace(": str =", " = ")[:-1] + " [|str|]\n"
					print("  Removing: static type str")
