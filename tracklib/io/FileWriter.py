...
class FileWriter:
	...
	def writeToFile(track, path, id_E=-1, id_N=-1, id_U=-1, id_T=-1, separator=",", h=0):
		'''
		...
		if fmt.h > 0:
			...
			if track.getSRID().upper() == "ENU":
				f.write(fmt.com + FileWriter.__printInOrder("E","N","U","time",O,fmt.separator) + "\n")
			if track.getSRID().upper() == "GEO":
				f.write(fmt.com + FileWriter.__printInOrder("lon","lat","h","time",O,fmt.separator) + "\n")
			if track.getSRID().upper() == "ECEF":
				f.write(fmt.com + FileWriter.__printInOrder("X","Y","Z","time",O,fmt.separator) + "\n")
	...
	def writeToFiles(trackCollection, pathDir, ext, id_E=-1, id_N=-1, id_U=-1, id_T=-1, separator=",", h=0):
		
		root = "track_output"
		
		for i in trackCollection.size():
			track = trackCollection.getTrack(i)
			path = pathDir + "/" + root + '{:04d}' + "." + ext
		if id_N == -1:
			if id_E == -1:
				writeToFile(track, path)   # Read by extension
			else:
				writeToFile(track, path, id_E)   # Read by name
		else:
			writeToFile(track, path, id_E, id_N, id_U, id_T, separator, h)  # Read from input parameters
