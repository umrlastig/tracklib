...
# filtrage et simplification. Pour memoire :
...
track = FileReader.readFromFile('data/trace0.gps') % 10
track.plot('kx')

def view(track, sym):
	track.plot(sym); print(track); print("Nb pts = ", track.size())
...
def test1(sym = 'r-'):
	temp = track.copy()
	temp.resample(delta=10)  
	view(temp, sym)

# Interpolation lineaire en mode temporel :  1 pt/s 
def test2(sym = 'r-'):
	temp = track.copy()
	temp.resample(delta=1, mode = itp.MODE_TEMPORAL)  
	view(temp, sym)

# Interpolation lineaire en mode temporel :  definition par timestamps
def test3(sym = 'r-'):
	temp = track.copy()
	a = int(track[0].timestamp.toAbsTime())
	b = int(track[-1].timestamp.toAbsTime())
